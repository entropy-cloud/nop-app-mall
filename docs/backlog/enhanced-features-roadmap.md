...
- 18. Dashboard 重做: `done`（`docs/plans/2026-06-28-1027-1-phase18-dashboard-redesign-plan.md`；4 看板 `@BizQuery`[getDashboardMetrics/getSalesTrend/getRealtimeOrders/getTodoAggregation] + AMIS chart 指标卡/趋势/实时订单流/待办聚合 4 区块 + owner doc 口径；nop-report 引擎按引擎定位归 P19 导出场景；goods 级安全库存 successor 已闭环：`docs/plans/2026-06-29-1045-2-goods-level-safety-stock-plan.md` done — Dashboard `getTodoAggregation.stockWarningDetails` 阈值解析改 per-goods `safetyStock` → global 两档[COALESCE SQL]，`StockWarningItemBean` 加 `safetyStock`/`thresholdSource`，明细表展示阈值来源）
- 19. 报表体系扩展: `done`（`docs/plans/2026-06-28-1027-2-phase19-report-system-extension-plan.md`；销售漏斗/用户分析[含生命周期]/商品分析/订单分析/优惠券分析 + 4 报表页 + CSV 导出[E1 抉择 CSV 兜底，nop-report 为 successor]；8 新增 `@BizQuery` 挂 LitemallOrderBizModel[E2 抉择] + SQL-lib `<c:if>` 条件查询 + Java cohort/RFM/lifecycle 分组；289 测试全绿；活动 ROI 归 P22，毛利 successor 已闭环：`docs/plans/2026-06-29-1921-1-goods-cost-price-margin-report-plan.md` done — `LitemallGoods.costPrice`(propId=25) + 商品分析毛利维度[成本额/毛利额/毛利率，D1/D3 口径] + getProductAnalysis 补 @Auth(admin) + product 主题 xlsx/pdf 毛利列 + admin 商品编辑页成本价字段；493 测试全绿）
...
- 22. 营销活动管理后台: `done`（`docs/plans/2026-06-28-0340-1-phase22-marketing-management-backend-plan.md` ORM-independent slice + `docs/plans/2026-06-28-1610-1-phase22-promotion-usage-model-gap-plan.md` ORM-dependent slice：PromotionUsage 实体 + flashSaleSessionId 列 + maxPerUser 强一致 + 按活动/按场次归因 + 效果看板秒杀面板；P22 deferred successor「满减参与记录退款回滚」已闭环：`docs/plans/2026-06-29-1921-2-promotion-usage-refund-rollback-plan.md` done — `releasePromotionUsage` helper + 6 处全额取消/退款镜像 hook[cancel/cancelExpiredOrders/售后 refund/售后 confirmReturnReceived/团购 refundGrouponOrder/拼团 refundMemberOrder] 软删除 PromotionUsage 释放 maxPerUser 额度，部分项退款保留 usage（满减为订单级折扣）；494 测试全绿）
...
- 35. 站内信/消息中心: `done`（`docs/plans/2026-06-28-0530-1-phase35-message-center-plan.md`；两份源 deferred 事件通知补全已闭环：`docs/plans/2026-06-29-1921-3-deferred-notification-triggers-plan.md` done — 积分过期前置预警推送[`send-points-expiry-reminders` 每日 job + 聚合 SYSTEM 站内信 + remindDays 配置 + 幂等] + 自提核销成功 ORDER 站内信[`verifyPickupOrder` afterCommit + 事件开关 + 幂等跳过分支不推送]；502 测试全绿；deferred successor「优惠券即将过期」已闭环：`docs/plans/2026-06-29-2330-2-coupon-expiry-notification-plan.md` done — `sendCouponExpiryReminders` `@BizMutation` + `send-coupon-expiry-reminders` 每日 job（`mall_coupon_expiry_remind_days` 缺省 3）+ 窗口查询[status=0 且 now<=endTime<=now+remindDays，endTime ASC] + 按 userId 聚合[Σ 张数 + 最早 endTime] + MARKETING 站内信[D1 营销促活] + 幂等[当日同 userId+msgType+title 不重复] + 5 IGraphQLEngine 测试；516 测试全绿；deferred successor「MARKETING 定向投放」已闭环：`docs/plans/2026-06-30-0044-2-segment-directed-marketing-push-plan.md` done — `sendSegmentMessage(segmentType,segmentValue,title,content)` `@BizMutation @Auth(admin)` 向选定分群[手工标签/RFM/生命周期]全部成员逐人写 MARKETING 站内信 + `collectRfmLifecycleMatches` 抽取（getSegmentMembers 零回归）+ 空分群返回 0[D3] + 前端 segment-push.page.yaml + action-auth 菜单 + 6 IGraphQLEngine 测试；542 测试全绿；deferred successor「拼团成功/评价奖励站内信」已闭环：`docs/plans/2026-06-30-0229-1-remaining-event-notifications-plan.md` done — `markPinTuanSuccess` 成团向团内全部成员各投 ORDER 站内信[主事务 flushSession 查成员 + afterCommit 零查询逐人投递 + ACTIVE→SUCCESS 状态守卫幂等] + `batchAuditComments` approve/`submitComment` 预审关两路径评价奖励各投 SYSTEM 站内信[reward>0 才投 + 事件开关 + afterCommit + PENDING→APPROVED 状态守卫幂等，非聚合] + 9 IGraphQLEngine 测试；551 测试全绿）
- 36. 商品运营增强: `done`（`docs/plans/2026-06-28-1027-3-phase36-goods-operations-enhancement-plan.md`；批量改价/改库存/上下架 + 导入[xlsx via ExcelHelper]/导出[CSV 兜底] + 库存预警[per-SKU `safeStock`+全局回退] + 评论工作台[回复/后置 Moderation]；前置审核状态机 successor 已闭环：`docs/plans/2026-06-28-2042-2-comment-pre-moderation-plan.md` done — `auditStatus` 字段+字典 + 预审开关 `mall_comment_pre_moderation` + `batchAuditComments`(approve/reject) + 公共查询过滤[doFind*ByQueryDirectly OR isNull/eq APPROVED] + 积分延迟至 approve 发放[幂等] + 工作台 auditStatus 筛选/批量通过拒绝；397 测试全绿；goods 级安全库存 successor 已闭环：`docs/plans/2026-06-29-1045-2-goods-level-safety-stock-plan.md` done — `LitemallGoods.safetyStock` 字段[propId=24 可空 INTEGER] + 工作台/Dashboard 阈值解析扩展三级优先[per-SKU safeStock → per-goods safetyStock → global mall_stock_threshold_tight] + `thresholdSource` 扩展 `safetyStock` 档 + DTO/SQL-lib/前端/owner doc 对齐）
