...
- 商城前台（AMIS 页面）：
  ...
  - 结算页（地址选择、优惠券选择、价格汇总、订单提交；团购上下文经 URL 参数 `grouponRulesId`/`grouponId` 透传到 submit）
  - 订单结果页（订单号展示、支付按钮）
  - 订单支付页（`/storefront-pay`：微信 Native 扫码二维码、轮询支付状态、模拟支付入口；4 处「立即付款」按钮入口接线）
  - 搜索发现页（关键字搜索、热门/默认关键字引导、搜索历史记录与清空）
  - 内容营销（专题列表页、专题详情页：富文本内容 + 关联商品卡片 + 收藏）
  - 促销（领券中心：可领取 + 我的优惠券按状态分组 + 兑换码；团购规则列表/详情、团购活动详情，开团/参团经加购→结算透传）
- 用户中心（AMIS 页面）：
  - 登录/注册页（登录、注册、密码重置）
  ...
  - 订单详情页（全量订单信息、价格明细）
  - 收货地址管理页（CRUD、默认地址切换）
  - 用户互动（收藏列表：商品/专题收藏；浏览足迹：查看/清空）
  - 售后（售后列表：状态展示与撤回；售后申请：订单级仅退款/退货退款）
  - 客服支持（反馈提交：基础文本反馈；FAQ：折叠面板浏览）
