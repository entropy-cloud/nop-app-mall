...
- **导出方式（E1）**：销售漏斗/商品分析/订单分析 3 报表已支持 nop-report 模板化导出（`LitemallOrder__exportReport`，xlsx/pdf）；用户分析/优惠券分析报表导出为 successor（同模式扩展，待用户/优惠券报表 xlsx/pdf 导出需求触发）。CSV 兜底保留为回退。
