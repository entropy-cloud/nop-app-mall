...
- 导出方式：**nop-report 模板化导出（xlsx/pdf）已引入**——`LitemallGoods__exportGoodsReport`（商品导出）与 `LitemallOrder__exportReport`（funnel/product/order 报表）经 `IReportEngine` + `.xpt.xml` 模板渲染，返回 `WebContentBean` 二进制下载；前端导出入口支持 xlsx/pdf/csv 三格式选择。CSV 兜底保留为回退（`exportGoods` CSV 不删除）。模板位于 `_vfs/nop/main/report/*.xpt.xml`。**注**：PDF 渲染中文需在 `_vfs/fonts/` 下提供 CJK TTF 字体（部署期关注），xlsx 原生支持中文。
...
- **待办聚合**：
  ...
  - 库存预警 = 在售商品中聚合库存（`SUM(LitemallGoodsProduct.number)`）≤ 阈值的商品计数，阈值解析：per-goods `LitemallGoods.safetyStock`（非空且 >0）→ 全局 `mall_stock_threshold_tight`（默认 10，经 `ILitemallSystemBiz.getConfig()` 读取，作为最终回退），明细含商品名 / 聚合库存 / safetyStock / thresholdSource。
...
- **导出方式（E1）**：5 大主题域报表已全部支持 nop-report 模板化导出（`LitemallOrder__exportReport`，xlsx/pdf）：销售漏斗/商品分析/订单分析（单 sheet）+ 用户分析（多 sheet：retention/rfm/lifecycle/repurchase）+ 优惠券分析（单 sheet）。CSV 兜底保留为回退。**注**：用户分析导出适配层将中文段名（lifecycle 新客/活跃/沉睡/流失、RFM 8 段）映射为 Latin 代码，与模板 Latin 表头先例一致；原 `getXxx` 口径不变。PDF 渲染中文需在 `_vfs/fonts/` 下提供 CJK TTF 字体（部署期关注）。
...
- 效果分析看板支持 `exportMarketingReport(renderType, startDate?, endDate?, promotionActivityId?, flashSaleId?, pinTuanActivityId?, couponId?)` `@BizQuery @Auth(admin)` 模板化导出（xlsx/pdf，单 xlsx 多 sheet：满减/秒杀/拼团/优惠券 4 面板各一 sheet），复用上述 4 效果 query、不改口径，与 funnel/product/order/user/coupon 统计页 nop-report 导出同模式（前端 `marketing-effect.page.yaml` 格式选择 + blob 按钮）。关闭 P22 Deferred「复杂报表导出（PDF/Excel）」successor。
...
| 导出商品 | `LitemallGoods__exportGoods` | 按筛选条件导出 CSV（兜底回退），含 SKU 关键字段 |
| 导出商品（模板） | `LitemallGoods__exportGoodsReport` | nop-report 模板化导出（xlsx/pdf），经 `IReportEngine` + `goods-export.xpt.xml` 渲染，返回 `WebContentBean` 二进制下载 |
| 导入商品 | `LitemallGoods__importGoods` | xlsx via `ExcelHelper.readSheet`（复用 batchShip 先例），按 goodsSn 匹配新增/更新，含字段校验 + 错误报告 |
| 库存预警 | `LitemallGoods__getStockWarningList` | SKU 粒度低库存列表，阈值三级优先：per-SKU `safeStock`（非空且 >0）→ per-goods `safetyStock`（非空且 >0）→ 全局 `mall_stock_threshold_tight`，按库存升序 |
...
| 批量评论前置审核 | `LitemallComment__batchAuditComments` | commentId 列表 + action=approve/reject，守卫 `auditStatus==PENDING`，approve→APPROVED（公开可见）+ 发放延迟积分（幂等）/ reject→REJECTED（公开不可见），结果报告（P36 successor） |
| 评论工作台列表 | `LitemallComment__getCommentReviewList` | 支持按 star/hasPicture/auditStatus/关键字/时间筛选 + 分页，供工作台消费 |
...
- 导出按可用写出入口抉择：已引入 nop-report 引擎（`IReportEngine` + `.xpt.xml` 模板）提供模板化 xlsx/pdf 导出（商品导出 + funnel/product/order 报表）；CSV 兜底保留为回退。

### 库存预警阈值来源（Decision G3）

阈值解析**三级优先**（per-SKU → per-goods → global），两条预警路径（P36 工作台 SKU 粒度 + P18 Dashboard goods 聚合）口径一致：

1. **per-SKU `LitemallGoodsProduct.safeStock`**（`orm.xml:806`，已存在，仅 SKU 粒度路径适用）— 非空且 >0 时优先。
2. **per-goods `LitemallGoods.safetyStock`**（`orm.xml:714`，propId=24，可空 INTEGER）— 非空且 >0 时为 goods 聚合级阈值；空或 0 视为未设置。
3. **全局 `mall_stock_threshold_tight`**（P38 已落地，默认 10）— 最终回退档。

- 工作台 `LitemallGoods__getStockWarningList`（SKU 粒度）解析顺序：safeStock → safetyStock → global；返回 `thresholdSource ∈ {"safeStock","safetyStock","global"}`。
- Dashboard `LitemallOrder__getTodoAggregation.stockWarningDetails`（goods 聚合）只适用 safetyStock → global 两档（聚合层无 per-SKU 概念），SQL 内 `COALESCE(NULLIF(g.SAFETY_STOCK,0), ${threshold})` 实现。
- 历史 goods 的 `safetyStock` 与 SKU 的 `safeStock` 多为空，初期预警依赖全局回退阈值；运营可逐步填充字段过渡到 per-goods / per-SKU 粒度。
- `mall_stock_threshold_tight` 同时承担 P38 展示侧「紧张档」上限语义与本运营侧安全库存回退阈值，二者复用同一全局键，不引入新键。

### 评论审核模型（Decision G4）

- **后置 Moderation（P36 已交付）：** 工作台列已发布评论，提供批量下架（置 `deleted=true`）/恢复（`deleted=false`）/批量回复（写 `adminContent`），复用既有字段，无 ORM 改动。
- **前置审核状态机（P36 successor，已交付）：** `LitemallComment` 新增可空列 `auditStatus`（字典 `mall/comment-audit-status`：PENDING=0/APPROVED=1/REJECTED=2）+ 预审开关配置 `mall_comment_pre_moderation`（`LitemallSystem`，"1"/"true"=ON，其余=OFF）。

#### 前置审核状态机

**字段与字典：**

- `LitemallComment.auditStatus`（propId=16，nullable INTEGER，`ext:dict="mall/comment-audit-status"`）。
- 字典 `mall/comment-audit-status`：PENDING(0, "待审核") / APPROVED(1, "已通过") / REJECTED(2, "已拒绝")。
- 列为可空；DB 不强制 default 0（避免历史行被误置 PENDING）。

**向后兼容语义（Decision A 抉择）：**

- **null = 已通过（公开可见）**，PENDING/REJECTED = 公开不可见。历史评论（null）无需迁移即公开。
- 预审开关 OFF 时新评论写 `auditStatus=null`（公开可见，行为同今）。
- 备选 B（mandatory + 默认 APPROVED + 迁移历史）被否——需数据迁移且与"null=通过"的宽松语义相比无额外收益。
- **残留风险：** 公共查询须显式过滤（`auditStatus 为空 OR APPROVED`），不能仅靠 `deleted` 管道。

**预审开关：**

- `mall_comment_pre_moderation`（`LitemallSystem` key=value 行）：OFF（empty/null/"0"/"false"/解析失败）→ 评论发布即审核通过；ON（"1"/"true"）→ 新评论 auditStatus=PENDING，公开不可见，待管理员审核。
- 防御解析模式复刻 `mall_points_comment_reward`（empty/null=off，解析失败=off）。

**状态流转：**

- 发布（`submitComment`）：预审 ON → PENDING；预审 OFF → null（公开）。
- 管理员审核（`batchAuditComments` action=approve/reject，`@Auth(roles="admin")`）：守卫 `auditStatus==PENDING`（非 PENDING 跳过），approve→APPROVED（公开可见）/reject→REJECTED（公开不可见）；catch-and-continue + 结果报告（复刻 `batchModerateComments` 先例）。

**公共可见性口径（Decision 公共查询过滤机制）：**

- 公共 `commentList`/`getCommentSummary` 仅返回/聚合 `auditStatus 为空 OR APPROVED` 的评论（PENDING/REJECTED 不公开）。
- `auditStatus` 的 `OR(isNull, eq APPROVED)` 复合过滤超出公共端 xmeta 允许的操作符面（与本文件其他 star/hasPicture 过滤同类约束），故公共查询改用 `doFindPageByQueryDirectly`/`doFindListByQueryDirectly` 构造过滤。
- **安全理由（必记）：** `directly` 变体绕过 xmeta 操作符限制但不绕过 `@Auth(publicAccess=true)` 的公开语义；查询仍由 BizModel 显式构造（valueId/type 维度 + auditStatus 过滤），不暴露任意字段筛选能力，与既有 `getCommentReviewList` 同一安全模型。
- 备选 B（扩 xmeta 允许 auditStatus 的 isNull/or）被否——为单字段放宽公共端 xmeta 操作符面更广、审计风险更高。
- `getCommentSummary` 的 goodRate/starDistribution/标签云仅基于公开可见集合（APPROVED/null）聚合，与 list 口径一致（避免汇总含未公开 PENDING 导致 goodRate 与可见列表不符）。

**预审期积分发放时机（Decision 积分延迟）：**

- 预审 ON 时积分延迟至审核通过发放：`batchAuditComments` approve 分支经 `I*LitemallPointsAccountBiz.earnPoints` 发放，复用同一 `sourceType=comment-reward`/`sourceId=comment.id` 保幂等；`earnPoints` 在 `(sourceType, sourceId)` 已存在时抛 `ERR_POINTS_DUPLICATE_EARN`，被 approve 分支捕获视为已发放（重审不重复发积分）。
- 预审 OFF 时维持 `submitComment` submit 即发（行为同今）。
- 备选 B（submit 即发 + REJECTED 时 clawback）被否——需新增扣减路径与对账复杂度，且 submit 即发与"审核通过才公开"语义不一致。
- **残留风险：** approve 分支承载积分发放编排（经 `I*LitemallPointsAccountBiz`），与状态推进需保持原子（同 `@BizMutation` 事务，失败行回滚但成功行不阻断）。

**工作台：**

- `comment-review.page.yaml` 增 `auditStatus` 筛选 select + crud 列（字典渲染：待审核=黄 / 已通过=绿 / 已拒绝=红 / null=已通过绿）+「批量通过」/「批量拒绝」bulkActions（调 `batchAuditComments` action=approve/reject，复刻既有"批量下架/恢复"的 feedback 结果表结构）。
...
| 支付通道启停 | → 出 | `order-and-cart.md` | `pay_channels` JSON 配置各通道操作开关（与通道能力位取交集决定收银台可见性） |
