...
> 本文件是 AGENTS.md Execution discipline #2 引用的校验清单，不是独立 skill prompt。

## 用途

每写完一个 BizModel 上的 public 方法（`@BizQuery`/`@BizMutation`/`@BizAction`）后，逐条校验。AGENTS.md 强制要求此步骤。private helper 方法不在校验范围内。

## 根因背景

反模式规则已写在 `docs-for-ai/` 必读文档中，但反复出现的问题是：

1. Agent 读了文档并标记 pre-flight checkbox `[x]`
2. 编码时没有逐方法对照规则
3. 闭包审计才发现违反（如参数用 `Object` 代替 `@DataBean`）

**根本原因：** "已读"不等于"已校验"。文档阅读和代码编写之间存在执行间隙。本 skill 强制在每个方法写完后插入校验步骤，关闭这个间隙。
...
| 12 | 错误码是否已定义在 `*Errors.java` 中？ | 是 → 通过。否 → **先定义错误码再使用** |
| 13 | 错误码描述是否使用中文（ErrorCode 模式）或英文（非 ErrorCode 模式）？ | 符合 → 通过 |

### 第五组：事务与注入（来自 `service-layer.md`）

| # | 检查项 | 通过条件 |
|---|--------|---------|
| 14 | `@BizMutation` 方法上是否有 `@Transactional`？ | 否 → 通过。是 → **移除**（`@BizMutation` 已自动事务） |
| 15 | `@Inject` 字段是否为 `private`？ | 否 → 通过。是 → **改为 package-private** |
| 16 | 方法名是否与 `ICrudBiz` 标准方法重名（`get`/`save`/`update`/`delete`/`findPage`）？ | 否 → 通过。是 → **重命名方法** |

### 第六组：平台工具类（来自 `common-java-helpers.md`）

| # | 检查项 | 通过条件 |
|---|--------|---------|
| 17 | 是否用了 `System.currentTimeMillis()`？ | 否 → 通过。是 → **改为 `CoreMetrics.currentTimeMillis()`** |
| 18 | 是否用了第三方 JSON 库？ | 否 → 通过。是 → **改为 `JsonTool`** |
| 19 | 是否用了 `Apache Commons StringXxx`？ | 否 → 通过。是 → **改为 `StringHelper`** |
...
- 不要用本清单替代阅读 `docs-for-ai/` 文档——本清单假设你已经读过
