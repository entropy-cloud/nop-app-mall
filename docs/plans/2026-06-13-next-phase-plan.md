...
- [ ] **Skill loading gate:** Load `nop-testing`. Read all mandatory docs listed in its routing table. List the docs read below.
  - Docs read: <to be filled during execution>
- [ ] **Add: TestLitemallIssueBizModel。** 测试 `listIssues`（公开 FAQ 列表）
- [ ] **Add: TestLitemallFeedbackBizModel。** 测试 `submitFeedback`（反馈提交）
- [ ] **Proof: 测试通过。** 所有新增测试通过

Exit Criteria:

- [ ] 2 个测试类通过 IGraphQLEngine 测试（Issue、Feedback）
- [ ] Phase 7/8/10 计划可以正式关闭（Topic/Ad 测试已存在）
- [ ] `docs/logs/` updated
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`, `nop-frontend-dev`, `nop-testing`. Read all mandatory docs listed in their routing tables. List the docs read below.
  - Docs read: <to be filled during execution>
- [ ] **Decision — 团购成功判定机制：** ORM LitemallGroupon.status 只有 3 个值（0=未付款、1=团购中、2=团购失败），没有"成功"状态值
...
- [ ] **Add: 扩展 `ILitemallGrouponRulesBiz` 接口。** 添加：
...
- [ ] **Add: `LitemallGrouponRulesBizModel` 实现。**
...
- [ ] **Add: 扩展 `ILitemallGrouponBiz` 接口。** 添加：
...
- [ ] **Add: `LitemallGrouponBizModel` 实现。**
...
- [ ] **Add: 团购后台页面定制。** 修改 `LitemallGrouponRules.view.xml` 和 `LitemallGroupon.view.xml`
- [ ] **Proof: 测试。** `TestLitemallGrouponBizModel`：
  - 测试上架/下架团购规则
  - 测试前台可用规则列表
  - 测试开团
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`, `nop-testing`. Read all mandatory docs. Selfcheck after each method.
  - Docs read: <to be filled during execution>
- [ ] **Modify: 扩展 `ILitemallOrderBiz` 接口。** 在 `submit` 方法签名中添加 `@Optional @Name("grouponRulesId") String grouponRulesId` 参数
- [ ] **Modify: `LitemallOrderBizModel.submit()` 集成团购价格。**
...
- [ ] **Decision — 团购下单路径区分开团和参团：** `submit()` 中的 `grouponRulesId` 参数同时处理开团和参团场景
...
- [ ] 订单提交可接受 @Optional grouponRulesId 和 @Optional grouponId 参数
- [ ] grouponPrice 正确从 actualPrice 扣减（不影响 orderPrice 计算）
- [ ] 开团/参团记录随订单正确创建
- [ ] 已有测试不受影响
- [ ] API 测试通过 IGraphQLEngine
- [ ] `docs/logs/` updated
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`. Read all mandatory docs.
  - Docs read: <to be filled during execution>
- [ ] **Decision — LitemallSystem vs NopSysVariable：** 确定系统配置管理方式
...
- [ ] **Modify: `LitemallOrderBizModel.submit()` 集成运费配置。** 从 LitemallSystem 读取运费配置，替代当前硬编码运费
- [ ] **Modify: `LitemallAddressBizModel` 集成地址上限配置。** 从 LitemallSystem 读取地址上限（当前硬编码 20）
- [ ] **Proof: 测试。** 验证配置读取正确
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`, `nop-testing`. Read all mandatory docs. Selfcheck after each method.
  - Docs read: <to be filled during execution>
- [ ] **Add: 引入 nop-job Maven 依赖。** 在 `app-mall-app/pom.xml` 中添加 `nop-job` 依赖
- [ ] **Add: 订单超时取消定时任务。** `cancelExpiredOrders()`：
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`, `nop-testing`. Read all mandatory docs.
  - Docs read: <to be filled during execution>
- [ ] **Add: 优惠券过期定时任务。** `expireCoupons()`：
  - 查询 status=0（未使用）且已过有效期的 LitemallCouponUser
  - 批量设置 status=2（已过期）
- [ ] **Add: 团购过期定时任务。** `expireGroupons()`：
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`. Read all mandatory docs.
  - Docs read: <to be filled during execution>
- [ ] **Audit: 管理员操作日志覆盖评估。** `MallLogManager` 已存在（当前仅在 `LitemallAftersaleBizModel` 中使用）。`system-configuration.md` 定义记录范围为：商品管理、发货/退款等订单操作、用户和管理员管理、系统配置变更。逐项评估并补充缺失的日志调用：
...
- [ ] **Add: 后台操作日志页面定制。** 修改 `LitemallLog.view.xml`
- [ ] **Proof: 验证。** 确认 `system-configuration.md` 记录范围内的操作有日志记录
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`, `nop-frontend-dev`, `nop-testing`. Read all mandatory docs.
  - Docs read: <to be filled during execution>
- [ ] **Add: 扩展 `ILitemallNoticeBiz` 接口。** 添加：
  - `listNotices(@Name("page") int page, @Name("pageSize") int pageSize, IServiceContext context)` — 前台公告列表（公开访问）
- [ ] **Add: `LitemallNoticeBizModel` 实现。**
  - `listNotices()` — 查询所有未删除公告，公开访问，按时间倒序
- [ ] **Add: 公告后台页面定制。** 修改 `LitemallNotice.view.xml`
- [ ] **Proof: 测试。** `TestLitemallNoticeBizModel`：验证 `listNotices` 通过 IGraphQLEngine 测试
...
- [ ] **Skill loading gate:** Load `nop-backend-dev`. Read all mandatory docs.
  - Docs read: <to be filled during execution>
- [ ] **Decision — LitemallStorage vs nop-integration-file-*：** 评估是否迁移到平台文件存储
...
- [ ] **Proof: 验证。** 确认文件上传和读取通过平台 `IFileService` 正常工作
...
- [ ] **Add: 关闭 Phase 7/8/10 计划。** 更新 `docs/plans/2026-06-12-phase-7-10-interactive-coupon-content-plan.md`，Phase 10 测试已补全
...
- [ ] Phase 10 测试补全（2 个测试类通过：Issue、Feedback；Topic/Ad 已存在）
- [ ] Phase 9 团购规则管理/开团参团/价格集成完成并通过测试
- [ ] Phase 11 系统配置/定时任务/日志/公告/文件存储完成并通过测试
