...
- [ ] **Skill loading gate:** 加载 `nop-backend-dev`，读完 routing table 中标记必读的文档。列出已读文档路径。
  - Docs read: `<fill after loading>`
- [ ] 读取 `LitemallAftersaleBizModel.java` 当前退款逻辑（约 line 108-116）
- [ ] 修改 `payService.refund()` 调用：捕获返回值 `PayRefundResponseBean`，检查退款是否成功。退款失败时抛出 `NopException`（使用 `AppMallErrors` 中已有或新增的错误码），阻止售后状态推进
- [ ] 新增错误码 `ERR_AFTERSALE_REFUND_FAILED`（如尚不存在）到 `AppMallErrors.java`
- [ ] 在 `TestLitemallAftersaleBizModel` 中补充测试用例：验证退款失败时售后状态不变且抛出正确错误码（通过 `IGraphQLEngine` 调用测试，因为 `refundForAdmin` 是 `@BizMutation` 方法）
- [ ] 验证：`./mvnw.cmd compile -DskipTests` 通过
- [ ] 验证：`./mvnw.cmd test` 通过（含新增测试用例）

Exit Criteria:

- [ ] `payService.refund()` 返回值被检查，失败时抛出 `NopException`
- [ ] 退款失败路径有测试覆盖（`TestLitemallAftersaleBizModel` 新增用例）
- [ ] 编译和测试通过
- [ ] `docs/logs/` 更新
...
- [ ] **Ask-first gate — XML 模型修改确认：** 本阶段需修改 `model/app-mall.orm.xml`（移除 3 个实体）。`ai-autonomy-policy.md` 明确要求 `model/*.orm.xml` 修改需人工批准。此步骤必须在执行前获得人工确认。确认后标记为 `[x]`。
- [ ] **Decision — 实体消除 vs 保留：** 确认是否现在消除 `LitemallAdmin`、`LitemallRole`、`LitemallPermission`。
...
- [ ] **Skill loading gate:** 加载 `nop-orm-modeler` 和 `nop-codegen-master`，读完必读文档。列出已读文档路径。
  - Docs read: `<fill after loading>`
- [ ] **Decision — 跨模型依赖处理（修正诊断）：** `LitemallUserInputBean`（由 `model/app-mall.orm.xml` 中的 codegen 生成）包含 `List<LitemallRoleInputBean> _relatedRoleList` 字段。经核实：
...
- [ ] 从 `model/app-mall.orm.xml` 移除 `LitemallAdmin`（约 line 170-195）、`LitemallPermission`（约 line 1023-1039）、`LitemallRole`（约 line 1070-1088）三个实体定义
- [ ] 验证 `codegen.sh`（或 `codegen.bat`）存在于项目根目录，确认可执行
- [ ] 运行 `codegen.sh`（或 `codegen.bat`）重新生成代码
- [ ] 删除 `app-mall-dao/src/main/java/app/mall/dao/entity/_gen/` 下的孤立 `_gen` 文件：`_LitemallAdmin.java`、`_LitemallRole.java`、`_LitemallPermission.java`（codegen 不会自动删除旧的 `_gen` 文件）
- [ ] 删除 `app-mall-dao/src/main/java/app/mall/dao/entity/` 下的 `LitemallAdmin.java`、`LitemallRole.java`、`LitemallPermission.java`
- [ ] 删除 `app-mall-dao/src/main/java/app/mall/biz/` 下的 `ILitemallAdminBiz.java`、`ILitemallRoleBiz.java`、`ILitemallPermissionBiz.java`
- [ ] 删除 `app-mall-service/src/main/java/app/mall/service/entity/` 下的 `LitemallAdminBizModel.java`、`LitemallRoleBizModel.java`、`LitemallPermissionBizModel.java`
- [ ] 删除 `app-mall-service/src/main/resources/_vfs/app/mall/model/` 下的自定义 xbiz 文件（非 `_gen` 前缀）
- [ ] 删除 `app-mall-web/src/main/resources/_vfs/app/mall/pages/` 下的 `LitemallAdmin/`、`LitemallRole/`、`LitemallPermission/` 目录（含 `_gen/` 和自定义 view）
- [ ] 清理 `app-mall-meta/_templates/` 下的对应模板文件
- [ ] 清理 `app-mall-api/src/main/java/app/mall/api/crud/` 下的 API 接口
- [ ] 清理 `app-mall-api/src/main/java/app/mall/api/beans/` 下的 Input/Output bean 类（codegen 会自动删除 Role 相关 bean，但需验证；同时删除孤立的 `LitemallUser*Bean.java` 和 `LitemallUserRole*Bean.java`）
- [ ] 清理 `app-mall-api/src/main/java/app/mall/api/crud/` 下的孤立 API 接口：`LitemallUserApi.java`、`LitemallUserRoleApi.java`（源实体已不存在，这些是孤立生成文件）
- [ ] 清理 `app-mall-meta/_templates/` 下的孤立模板：`_LitemallUserRole.json`（如存在）
- [ ] 全项目 grep 验证：`grep -r "LitemallAdmin\|LitemallRole\|LitemallPermission" app-mall-service/ app-mall-web/ app-mall-api/ app-mall-dao/ app-mall-meta/ --include="*.java" --include="*.xml"` — 预期零命中（排除 `_gen` 目录中已被删除的文件）
- [ ] 修复所有编译引用（如 grep 发现残余引用）
- [ ] 验证：`./mvnw.cmd clean compile -DskipTests` 通过
- [ ] 验证：`./mvnw.cmd test` 已有测试全部通过
- [ ] 验证：应用可正常启动（`java -Dfile.encoding=UTF8 -Dquarkus.profile=dev -jar app-mall-app/target/app-mall-app-1.0-SNAPSHOT-runner.jar` 启动无报错）

Exit Criteria:

- [ ] 三个实体从 ORM 模型中移除
- [ ] 确认 delta 模型无需修改；codegen 重新生成后 User bean 不再引用 Role 类型
- [ ] 代码重新生成成功，`_gen` 孤立文件已手动删除
- [ ] 对应的 BizModel、view、模板、接口、bean 文件清理完毕
- [ ] 全项目 grep 验证通过（零命中，排除已删除的 `_gen` 文件）
- [ ] 编译和测试通过
- [ ] 应用可正常启动
- [ ] `docs/logs/` 更新
...
- [ ] **Skill loading gate:** 加载 `nop-backend-dev`。列出已读文档路径。
  - Docs read: `<fill after loading>`
- [ ] `LitemallOrderBizModel.java` — 在 `goodsProductMapper` 注入处添加注释：说明 `reduceStock`/`addStock` 为原子 SQL UPDATE 操作，用于并发安全的库存扣减/回补，无法通过 I*Biz 接口表达
- [ ] `LitemallAftersaleBizModel.java` — 在 `goodsProductMapper` 注入处添加注释：说明售后退款后库存回补使用原子 SQL UPDATE
- [ ] `LitemallGoodsBizModel.java` — 在 `goodsMapper` 注入处添加注释：说明 `syncCartProduct` 为批量 SQL 同步操作

Exit Criteria:

- [ ] 3 处 `@SqlLibMapper` 注入均有注释说明理由
- [ ] 编译通过
- [ ] `docs/logs/` 更新
...
- [ ] **Skill loading gate:** 加载 `nop-frontend-dev`，读完必读文档。列出已读文档路径。
  - Docs read: `<fill after loading>`

#### 4A. LitemallAddress 管理页面

- [ ] 定制 `LitemallAddress.view.xml`：添加列表列（userId、name、tel、province/city/area/detail、isDefault）、查询表单（userId）、查看表单字段布局

#### 4B. LitemallCouponUser 管理页面

- [ ] 定制 `LitemallCouponUser.view.xml`：添加列表列（userId、couponId、status、usedTime、orderId）、查询表单（userId、status）、查看表单字段布局

#### 4C. LitemallKeyword 管理页面

- [ ] 定制 `LitemallKeyword.view.xml`：添加列表列（keyword、isDefault、isHot）、查询表单（keyword、isDefault）、编辑表单字段

Exit Criteria:

- [ ] 3 个 view.xml 有有意义的列定义和表单布局
...
- [ ] **M4 — 对齐订单状态标签：** 更新 `docs/design/order-and-cart.md` 约第 73 行的状态表，使用与 `model/app-mall.orm.xml` 的 `mall/order-status` 字典一致的中文标签（以 ORM 字典为准）。补充 ORM 中存在但设计文档缺失的 `204 GROUPON_EXPIRED`（"已超时团购"）
- [ ] **M3+M6 — 关闭中文化计划：**
...
- [ ] `order-and-cart.md` 状态标签与 ORM 字典一致，`204 GROUPON_EXPIRED` 已补充
- [ ] 中文化计划状态为 `completed`，Closure Gates 已勾选
- [ ] `docs/logs/` 更新
...
- [ ] relevant docs are aligned（设计文档、ORM 字典、计划状态一致）
- [ ] verification has run: `./mvnw.cmd clean compile -DskipTests` + `./mvnw.cmd test`
- [ ] 所有新增/修改的 `@BizMutation`/`@BizQuery` 方法通过 `IGraphQLEngine` 测试（Phase 1 修改了 `LitemallAftersaleBizModel` 的退款方法错误处理行为，需通过测试验证新的错误抛出路径）
...
Status Note: `<fill after closure>`

Closure Audit Evidence:

- Reviewer / Agent: `<independent subagent>`
- Evidence: `<fill after closure>`
