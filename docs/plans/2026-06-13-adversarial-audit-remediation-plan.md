...
- [ ] **AR-1 [P0]:** 重写 `model/app-mall.api.xml`，修正所有元数据：`appName=app-mall`、`serviceModuleName=app-mall-service`、`servicePackageName=app.mall.service`、`apiModuleName=app-mall-api`、`mavenGroupId=io.nop.app`（与 orm.xml 一致）、`mavenArtifactId=app-mall`、`mavenVersion=1.0-SNAPSHOT`、`displayName="商城服务"`
- [ ] **AR-2 [P1]:** `model/app-mall.orm.xml:1102` — 将 `LitemallTopic.readCount` 改为 `stdDataType="int" stdSqlType="INTEGER"`，移除 `precision="255"`
- [ ] **AR-3 [P1]:** `model/app-mall.orm.xml:731` — 将 `LitemallGrouponRules.discount` 的 `precision` 从 `63` 改为 `10`，`scale` 从 `0` 改为 `2`
- [ ] **AR-6 [P2]:** `model/app-mall.orm.xml:265,490,747` — 将三个 `to-one` 关系的 `displayName` 从 `"订单"` 改为 `"商品"`
- [ ] **AR-20 [P3]:** `model/nop-auth-delta.orm.xml:4` — 将 `ext:mavenVersion` 从 `1.0.0-SNAPSHOT` 改为 `1.0-SNAPSHOT`
- [ ] **Verification:** `./mvnw compile -DskipTests` 编译通过

Exit Criteria:

- [ ] `model/app-mall.api.xml` 所有元数据指向 app-mall 项目
- [ ] `readCount` 为 `stdDataType="int"`、`discount` 为 `precision="10" scale="2"`
- [ ] 三个关系 `displayName="商品"`
- [ ] 版本字符串统一为 `1.0-SNAPSHOT`
- [ ] `./mvnw compile -DskipTests` 通过
- [ ] No owner-doc update required（模型文件本身就是 source of truth）
...
- [ ] **Skill loading gate:** 加载 `nop-backend-dev` skill，读取其路由表中所有必读文档。列出已读文档路径。
  - Docs read: <list paths here>
- [ ] **AR-5 [P1]:** `LitemallCart.java:19` — 在 `AppMallErrors.java` 中添加错误码 `ERR_CART_NUMBER_ZERO`（`nop.err.mall.cart.number-zero`，中文描述"购物车商品数量为零"），将 `throw new IllegalStateException(...)` 改为 `throw new NopException(ERR_CART_NUMBER_ZERO)`
- [ ] **AR-7 [P2]:** `MallLogManager.java:91` — 删除重复的 `log.setType(type)` 行
- [ ] **AR-8 [P2]:** `LitemallOrderBizModel.java:536` — 注入 `INopAuthUserBiz userBiz`，将 `daoProvider().daoFor(NopAuthUser.class).findAllByQuery(query)` 改为 `userBiz.findList(query, null, context)`，移除未使用的 `NopAuthUser` import（如果不再需要）
- [ ] **AR-9 [P2]:** `LitemallTopicBizModel.java:54-58` — 在 `AppMallErrors.java` 中添加 `ERR_TOPIC_NOT_FOUND`，将 `return null` 改为 `throw new NopException(ERR_TOPIC_NOT_FOUND).param("id", id)`
- [ ] **AR-10 [P1]:** `LoginApiExBizModel.java:222-243` — 重构 `dispatchRegistrationCoupons`：不再修改 `ContextProvider` 的 userId，改为在 `ILitemallCouponUserBiz` 接口（`app-mall-dao`）和 `LitemallCouponUserBizModel`（`app-mall-service`）中新增 `claimCouponForUser(@Name("couponId") String couponId, @Name("userId") String userId, IServiceContext context)` 方法。**Decision:** 该方法使用 `@BizAction`（非公开，内部调用），而非 `@BizMutation`，因为它是注册流程的内部步骤，不应独立暴露为 GraphQL 端点。内部直接使用传入的 `userId` 而非 `context.getUserId()`
- [ ] **AR-10 test:** 为 `claimCouponForUser` 编写测试，通过 `ILitemallCouponUserBiz` 接口直接调用（因为使用 `@BizAction`，不走 GraphQL）。测试内容：正常领取、领取不存在的券、重复领取被拒
- [ ] **AR-13 [P2]:** `LitemallOrderBizModel.java:245` — 将 `actualPrice <= 0` 的自动支付逻辑改为：先检查 `actualPrice < 0` 时抛出 `NopException(ERR_ORDER_PRICE_INVALID)`（在 `AppMallErrors` 中新增），`actualPrice == 0` 时维持自动支付逻辑
- [ ] **AR-15 [P2]:** `LitemallGrouponBizModel.java:178-184` — 删除 `findList(query, null, context)` 死代码（返回值未使用）
- [ ] **AR-18 [P3]:** `LoginApiExBizModel.java:132` — 将 `"gender", 1` 改为 `"gender", 0`
- [ ] **Verification:** `./mvnw compile -DskipTests` 编译通过

Exit Criteria:

- [ ] `LitemallCart.validateForCheckout()` 抛出 `NopException`
- [ ] `MallLogManager` 无重复 `setType` 调用
- [ ] `LitemallOrderBizModel.getUserStatistics()` 使用 `INopAuthUserBiz` 接口
- [ ] `LitemallTopicBizModel.frontDetail()` 抛出异常而非返回 null
- [ ] `dispatchRegistrationCoupons` 不修改 `ContextProvider` 的 userId
- [ ] `LitemallOrderBizModel.submit()` 拒绝负价格订单
- [ ] `LitemallGrouponBizModel.grouponDetail()` 无死代码
- [ ] 注册用户 gender 默认为 0（未知）
- [ ] `./mvnw compile -DskipTests` 通过
- [ ] `docs/logs/` updated
...
- [ ] **Skill loading gate:** 加载 `nop-backend-dev` skill，读取其路由表中所有必读文档。列出已读文档路径。
  - Docs read: <list paths here>
- [ ] **AR-12 [P1]:** `application.yaml:22` — 将 `enc-key` 改为 `enc-key: ${JWT_ENC_KEY}`（无默认值，强制通过环境变量设置）。在 `%dev` profile 中保留当前开发密钥 `enc-key: ${JWT_ENC_KEY:57adcda2601e429f8422d37bfa07166e}`。同时在 default 配置块中添加注释 `# WARNING: 生产部署必须设置环境变量 JWT_ENC_KEY`
- [ ] **AR-25 [P1]:** `application.yaml` — 将 default profile 中的以下安全敏感设置移入 `%dev` profile（仅 dev 生效）：
...
- [ ] **AR-21 [P2]:** `app-mall-api/pom.xml` — 添加 `<parent>` 引用 `app-mall`（groupId=`io.nop.app`, artifactId=`app-mall`, version=`1.0-SNAPSHOT`），移除独立的 `<properties>` 部分。**Decision:** 显式确认父 POM 链（app-mall → nop-entropy）中 `java.version` 为 17，api 模块从 11 升级到 17 是正确行为（项目已使用 Java 17+ 特性）。`<build>` 中的 `maven-site-plugin` 和 `maven-project-info-reports-plugin` 被显式丢弃（非项目必需，其他模块均未使用这些插件）
- [ ] **Verification:** `./mvnw compile -DskipTests` 编译通过

Exit Criteria:

- [ ] JWT enc-key 使用环境变量占位符
- [ ] default profile 中 `init-database-schema`、`support-debug`、`schema-introspection`、`allow-create-default-user` 均为安全值（false）
- [ ] `%dev` profile 保留开发友好的 true 设置
- [ ] `app-mall-api/pom.xml` 继承父 POM，无独立 properties
- [ ] `./mvnw compile -DskipTests` 通过
- [ ] `docs/logs/` updated
...
- [ ] **Skill loading gate:** 加载 `nop-frontend-dev` 和 `nop-backend-dev` skill，读取其路由表中所有必读文档。列出已读文档路径。
  - Docs read: <list paths here>
- [ ] **AR-16 [P1]:** **Decision:** 删除整个 `LitemallUser` 页面目录（包括 `_gen/`、`LitemallUser.view.xml`、`picker.page.yaml`、`main.page.yaml`、`LitemallUser.lib.xjs`），因为用户管理已合并到 `NopAuthUser`。`LitemallUser` 无独立 BizModel，保留页面无意义。如果未来需要独立用户管理页面，应通过 `NopAuthUser` 的 delta 定制实现。删除前确认无其他 view.xml 引用 LitemallUser 的 picker
- [ ] **AR-17 [P2]:** `LitemallComment/LitemallComment.view.xml` — 在 `admin-reply` 的 mutation API URL 中添加 `?id=$id` 参数，或在 `admin-reply-form` 的 layout 中添加隐藏字段 `id`
- [ ] **AR-19 [P2]:** 在 `docs/design/user-and-address.md` 的 Delta 扩展字段章节中添加一条记录：说明 `clientId` 列被删除的原因（商场应用不使用 nop-auth 原生的 clientId 设备追踪功能，该字段仅用于 nop-auth 内部的多端登录控制场景，商场用户通过手机号/用户名登录不需要此字段）
- [ ] **AR-23 [P3]:** 删除 `NopAuthUserEx2BizModel.java`；从 `auth-service.beans.xml` delta 中移除 `NopAuthUserEx2BizModel` 的 bean 注册；从 `NopAuthUser.xbiz` 中移除 `extAction3` 测试函数定义
- [ ] **Verification:** `./mvnw compile -DskipTests` 编译通过

Exit Criteria:

- [ ] `LitemallUser` 页面不再引用不存在的 `LitemallUser` BizModel
- [ ] `adminReply` mutation 能正确传递 `id` 参数
- [ ] `docs/design/user-and-address.md` 记录了 `clientId` 删除原因
- [ ] 无 `NopAuthUserEx2BizModel` 和 `extAction3` 残留
- [ ] `./mvnw compile -DskipTests` 通过
- [ ] `docs/logs/` updated
...
- [ ] **Skill loading gate:** 加载 `nop-testing` skill，读取其路由表中所有必读文档。列出已读文档路径。
  - Docs read: <list paths here>
- [ ] `./mvnw compile -DskipTests` 编译通过
- [ ] `./mvnw test` 运行所有现有测试，全部通过
- [ ] 验证 `LitemallTopic.frontDetail` 对不存在 ID 抛出异常（通过检查已有测试文件 `TestLitemallTopicBizModel.java` 是否需要更新 snapshot）
- [ ] 验证 `LitemallCart.validateForCheckout` 使用 NopException（通过检查引用此方法的测试）
- [ ] 更新 `docs/logs/2026/06-13.md` 记录本次修正
...
- [ ] relevant docs are aligned（`docs/design/user-and-address.md` 已更新 clientId 删除原因）
- [ ] verification has run: `./mvnw compile -DskipTests` && `./mvnw test`
- [ ] 新增 `@BizAction` 方法 `claimCouponForUser` 通过 `ILitemallCouponUserBiz` 接口测试（不走 GraphQL）
- [ ] no in-scope item downgraded to deferred/follow-up
- [ ] plan audit passed before implementation
- [ ] each phase has `Required Skill` listed
- [ ] skill loading verification: each phase scanned available skills, loaded all matching skills, read ALL mandatory docs
